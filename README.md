...
- 会话导航：Codex 任务可切换到对应桌面对话；Claude/tmux 跳转会复核目标 pane 与窗口，未确认时保留界面并提示手动切换
- 刘海内交互：查看聊天记录、向 Codex 继续发送消息，并用按钮或 `⌘Y` / `⌘N` 审批工具请求
- 逐会话审批：单次审批、自动审批、完全信任；默认仍为单次审批
- 稳定悬停：悬停预览可延迟展开和自动收起，点击进入的聊天或菜单不会被旧计时器误关
- 全局展开／收起快捷键：在「设置 → 快捷键」录制自己的组合键，默认未绑定；按键松开时切换一次，保留聊天草稿，不执行审批；快速关闭后旧焦点回调不会重新激活刘海
- 键盘切换会话：可分别设置上一个／下一个会话组合键，按任务列表顺序循环打开刘海内聊天；取当前可见会话，跳过已结束任务；同一组合键分配多个动作时显示冲突并暂停这些动作，不跳转终端、不自动回答
- 紧凑问题模式：可关闭问题自动展开，只保留小刘海待处理提示；审批与计划仍即时展开
- 问题草稿：收起刘海或切换任务后保留选项、自定义多行答案和页码；按会话/请求隔离，题目变化或请求结束后清理，仅在内存保存
- 完整答案回传：Bridge 支持分段到达的长答案和中文/emoji，不再截断于 4 KiB；无效或超时回包回退原生处理，不伪造批准
- 时序安全提醒：完成提示按真实回合边界只触发一次，重启不补播旧提示；可选跟进提醒会对仍未处理的请求或尚未打开的完成结果再提示一次，并在解决、查看或终端已聚焦时取消
- 静默时段：可按本地时间设置每日免打扰区间，支持跨午夜；仅关闭自动声音，不隐藏审批、问题和完成状态
- 静默场景：锁屏、显示器/系统休眠或登录会话失活时自动静音；交叠场景按独立状态恢复，避免过早重新响铃
- 静默规则：按项目名/路径、首条提示词/标题、Agent/CLI 或工具名匹配；规则即时显示当前会话命中数，并抑制匹配会话的自动展开、完成弹跳、声音与跟进提醒
- 分事件声音：完成、审批/计划、问题、跟进提醒独立选择系统声音；自动声音总开关不隐藏设置，试听只在主动点击时播放；新审批和问题默认无声，跟进音默认沿用完成音
- 提醒音量：0–100% 独立调节，不改变系统音量；提醒与试听共用音量，0% 不启动音频播放；连续试听会停止上一段声音
- 自定义提醒音：每类事件可导入 WAV、MP3、AIFF 文件（最多 10 MB / 30 秒）；后台校验并保存本地副本，取消或失败保留原设置；跟进提醒可继承自定义完成音
- 本地设置：中英文、常驻/智能隐藏、简略/详细、尺寸和动画预览
- 原生设置布局：通用、外观、通知、快捷键、用量、系统六个分类；系统侧栏与强调色，分组控件随系统外观调整，预览只出现在外观页；切页回到顶部，窗口标题标明当前分类
- 本地优先：不包含产品分析 SDK，不上传会话遥测

在「设置 → 通知 → 静默规则」中，可查看实时命中数，点击铅笔修改已有规则。
保存后立即生效，取消或按 Escape 放弃草稿；编辑会保留原规则的启用状态。

在「设置 → 系统」中配置分事件声音。跟进音可选择“跟随任务完成”或独立声音；
关闭自动提醒音不清空选择，也不会在重新开启后补播已观察的请求。
自动声音仍遵循静默时段、静默场景和会话静默规则；总开关不影响视觉状态和手动试听。
音量默认保留原先的 100%；拖动滑块只调节音量，不自动试听。关闭自动提醒音会停止
当前自动提醒，手动试听仅受音量控制；把音量调回非零不会补播已停止的声音。
每个声音选项旁的文件按钮用于导入，选择文件不会自动播放。副本保存在
`~/Library/Application Support/Agent Notch/Sounds/`，不会修改原文件；替换选择时仅清理
不再被任何事件引用的旧副本。文件不可用时保持原选择并提示重新导入，不回退播放其他声音。
自动提醒在播放期间也跟踪原会话和事件：任务继续、请求解决或被新事件替代后停止旧声音；
进入锁屏等静默场景或新增命中静默规则时也会停止。恢复状态不会续播，手动试听不受这些自动提醒规则影响。
静默时段使用单次边界定时任务，即使没有新的 Agent 事件也能到点停止自动声音；
改时间、改时区或从休眠唤醒后重新计算，关闭静默时段或选择全天静默时不保留边界定时任务。
