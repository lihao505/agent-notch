...
struct NotchMenuView: View {
    ...
    @State private var isUpdatingHooks = false
    @State private var errorMessage: String?

    var body: some View {
        NotchQuickControls(
            language: preferences.language,
            idleBehavior: $preferences.idleBehavior,
            ...
            showUsageLimits: $preferences.showUsageLimits,
            launchAtLogin: Binding(
                get: { launchAtLogin },
                set: { if $0 != launchAtLogin { toggleLaunchAtLogin() } }
            ),
            hooksInstalled: Binding(
                get: { hooksInstalled },
                set: { if $0 != hooksInstalled { toggleHooks() } }
            ),
            hooksNeedRepair: hooksNeedRepair,
            isUpdatingHooks: isUpdatingHooks,
            errorMessage: errorMessage,
            onBack: { viewModel.toggleMenu() },
            onToggleLanguage: {
                preferences.language = preferences.language == .english
                    ? .simplifiedChinese : .english
            },
            onOpenSettings: {
                AppDelegate.shared?.showDetailedSettings(afterCollapsing: viewModel)
            },
            onQuit: { AppDelegate.shared?.quitCompletely() }
        )
    ...
    private func toggleLaunchAtLogin() {
        ...
        do {
            ...
            if SMAppService.mainApp.status == .requiresApproval {
                errorMessage = t(
                    "Allow Agent Notch in System Settings → Login Items.",
                    "请在系统设置 → 登录项中允许 Agent Notch。"
                )
            }
        } catch {
            launchAtLogin = SMAppService.mainApp.status == .enabled
            errorMessage = t("Could not update login item. Try again in System Settings.",
                             "无法更新登录项，请在系统设置中重试。")
        }
    }

    private func toggleHooks() {
        guard !isUpdatingHooks else { return }
        isUpdatingHooks = true
        errorMessage = nil
        ...
        Task {
            ...
            if !succeeded {
                errorMessage = t("Bridge update failed. Open All Settings to review integrations.",
                                 "桥接更新未完成，请打开完整设置检查集成。")
            }
        }
    }
